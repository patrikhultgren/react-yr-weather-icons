...
describe("ClearSkyNight", () => {
  it("renders without crashing", () => {
    render(<ClearSkyNight />);
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toBeInTheDocument();
  });

  it("renders title when provided", () => {
    render(<ClearSkyNight title="ClearSkyNight title" titleId="clearskynight-title" />);
    const title = screen.getByTitle("ClearSkyNight title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "clearskynight-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<ClearSkyNight titleId="clearsky_night-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<ClearSkyNight width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<ClearSkyNight className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
