...
describe("ClearSkyDay", () => {
  ...
  it("renders title when provided", () => {
    render(<ClearSkyDay title="ClearSkyDay title" titleId="clearskyday-title" />);
    const title = screen.getByTitle("ClearSkyDay title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "clearskyday-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<ClearSkyDay titleId="clearsky_day-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<ClearSkyDay width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<ClearSkyDay className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
