...
import SnowShowersNight from "../snowshowers_night";

describe("SnowShowersNight", () => {
  it("renders without crashing", () => {
    render(<SnowShowersNight />);
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toBeInTheDocument();
  });

  it("renders title when provided", () => {
    render(<SnowShowersNight title="SnowShowersNight title" titleId="snowshowersnight-title" />);
    const title = screen.getByTitle("SnowShowersNight title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "snowshowersnight-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<SnowShowersNight titleId="snowshowers_night-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<SnowShowersNight width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<SnowShowersNight className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
