...
describe("HeavySnow", () => {
  ...
  it("renders title when provided", () => {
    render(<HeavySnow title="HeavySnow title" titleId="heavysnow-title" />);
    const title = screen.getByTitle("HeavySnow title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "heavysnow-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<HeavySnow titleId="heavysnow-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<HeavySnow width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<HeavySnow className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
