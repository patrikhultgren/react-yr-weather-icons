...
import RainShowersDay from "../rainshowers_day";

describe("RainShowersDay", () => {
  it("renders without crashing", () => {
    render(<RainShowersDay />);
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toBeInTheDocument();
  });

  it("renders title when provided", () => {
    render(<RainShowersDay title="RainShowersDay title" titleId="rainshowersday-title" />);
    const title = screen.getByTitle("RainShowersDay title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "rainshowersday-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<RainShowersDay titleId="rainshowers_day-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<RainShowersDay width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<RainShowersDay className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
