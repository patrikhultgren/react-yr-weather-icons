...
describe("LightRain", () => {
  ...
  it("renders title when provided", () => {
    render(<LightRain title="LightRain title" titleId="lightrain-title" />);
    const title = screen.getByTitle("LightRain title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "lightrain-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<LightRain titleId="lightrain-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<LightRain width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<LightRain className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
