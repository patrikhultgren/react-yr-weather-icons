...
import LightSleet from "../lightsleet";
...
describe("LightSleet", () => {
  ...
  it("renders title when provided", () => {
    render(<LightSleet title="LightSleet title" titleId="lightsleet-title" />);
    const title = screen.getByTitle("LightSleet title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "lightsleet-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<LightSleet titleId="lightsleet-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<LightSleet width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<LightSleet className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
