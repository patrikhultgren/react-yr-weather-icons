...
import SleetShowersDay from "../sleetshowers_day";

describe("SleetShowersDay", () => {
  it("renders without crashing", () => {
    render(<SleetShowersDay />);
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toBeInTheDocument();
  });

  it("renders title when provided", () => {
    render(<SleetShowersDay title="SleetShowersDay title" titleId="sleetshowersday-title" />);
    const title = screen.getByTitle("SleetShowersDay title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "sleetshowersday-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<SleetShowersDay titleId="sleetshowers_day-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<SleetShowersDay width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<SleetShowersDay className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
