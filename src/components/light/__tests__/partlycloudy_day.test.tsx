...
import PartlyCloudyDay from "../partlycloudy_day";

describe("PartlyCloudyDay", () => {
  it("renders without crashing", () => {
    render(<PartlyCloudyDay />);
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toBeInTheDocument();
  });

  it("renders title when provided", () => {
    render(<PartlyCloudyDay title="PartlyCloudyDay title" titleId="partlycloudyday-title" />);
    const title = screen.getByTitle("PartlyCloudyDay title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "partlycloudyday-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<PartlyCloudyDay titleId="partlycloudy_day-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<PartlyCloudyDay width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<PartlyCloudyDay className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
