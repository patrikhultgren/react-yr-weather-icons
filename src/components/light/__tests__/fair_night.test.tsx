...
describe("FairNight", () => {
  ...
  it("renders title when provided", () => {
    render(<FairNight title="FairNight title" titleId="fairnight-title" />);
    const title = screen.getByTitle("FairNight title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "fairnight-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<FairNight titleId="fair_night-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<FairNight width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<FairNight className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
