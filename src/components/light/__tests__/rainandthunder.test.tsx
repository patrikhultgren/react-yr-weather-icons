...
import RainandThunder from "../rainandthunder";

describe("RainandThunder", () => {
  it("renders without crashing", () => {
    render(<RainandThunder />);
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toBeInTheDocument();
  });

  it("renders title when provided", () => {
    render(<RainandThunder title="RainandThunder title" titleId="rainandthunder-title" />);
    const title = screen.getByTitle("RainandThunder title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "rainandthunder-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<RainandThunder titleId="rainandthunder-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<RainandThunder width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<RainandThunder className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
