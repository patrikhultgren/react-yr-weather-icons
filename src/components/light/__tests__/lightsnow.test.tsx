...
describe("LightSnow", () => {
  ...
  it("renders title when provided", () => {
    render(<LightSnow title="LightSnow title" titleId="lightsnow-title" />);
    const title = screen.getByTitle("LightSnow title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "lightsnow-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<LightSnow titleId="lightsnow-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<LightSnow width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<LightSnow className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
