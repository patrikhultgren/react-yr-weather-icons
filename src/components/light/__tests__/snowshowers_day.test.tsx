...
import SnowShowersDay from "../snowshowers_day";

describe("SnowShowersDay", () => {
  it("renders without crashing", () => {
    render(<SnowShowersDay />);
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toBeInTheDocument();
  });

  it("renders title when provided", () => {
    render(<SnowShowersDay title="SnowShowersDay title" titleId="snowshowersday-title" />);
    const title = screen.getByTitle("SnowShowersDay title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "snowshowersday-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<SnowShowersDay titleId="snowshowers_day-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<SnowShowersDay width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<SnowShowersDay className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
