...
import SnowAndThunder from "../snowandthunder";

describe("SnowAndThunder", () => {
  it("renders without crashing", () => {
    render(<SnowAndThunder />);
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toBeInTheDocument();
  });

  it("renders title when provided", () => {
    render(<SnowAndThunder title="SnowAndThunder title" titleId="snowandthunder-title" />);
    const title = screen.getByTitle("SnowAndThunder title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "snowandthunder-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<SnowAndThunder titleId="snowandthunder-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<SnowAndThunder width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<SnowAndThunder className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
