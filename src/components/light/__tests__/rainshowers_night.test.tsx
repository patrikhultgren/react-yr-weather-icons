...
import RainShowersNight from "../rainshowers_night";

describe("RainShowersNight", () => {
  it("renders without crashing", () => {
    render(<RainShowersNight />);
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toBeInTheDocument();
  });

  it("renders title when provided", () => {
    render(<RainShowersNight title="RainShowersNight title" titleId="rainshowersnight-title" />);
    const title = screen.getByTitle("RainShowersNight title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "rainshowersnight-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<RainShowersNight titleId="rainshowers_night-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<RainShowersNight width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<RainShowersNight className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
