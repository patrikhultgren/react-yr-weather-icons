...
describe("HeavyRain", () => {
  ...
  it("renders title when provided", () => {
    render(<HeavyRain title="HeavyRain title" titleId="heavyrain-title" />);
    const title = screen.getByTitle("HeavyRain title");
    expect(title).toBeInTheDocument();
    const svg = screen.getByRole("img", { hidden: true });
    expect(svg).toHaveAttribute("aria-labelledby", "heavyrain-title");
  });

  it("does not render title element if title prop is not provided", () => {
    render(<HeavyRain titleId="heavyrain-title" />);
    const title = screen.queryByTitle(/./);
    expect(title).toBeNull();
  });

  it("accepts and applies width and height props", () => {
    render(<HeavyRain width="100px" height="150px" />);
    const svg = screen.getByRole("img", { hidden: true });
  ...
  it("accepts and applies className prop", () => {
    render(<HeavyRain className="my-custom-class" />);
    const svg = screen.getByRole("img", { hidden: true });
